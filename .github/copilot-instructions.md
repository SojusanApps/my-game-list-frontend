...
**Key Technologies:** React 19, TypeScript, Vite, TanStack Query v5, TanStack Router v1, Mantine v8, @mantine/form + Zod, OpenAPI-TS
...
Route definitions are automatically generated by the Vite plugin (`src/routeTree.gen.ts`).

- Thin route files are located in `src/routes/`.
- Thick page components remain in their respective features (`src/features/*/pages/`).

**Route Protection (`beforeLoad` example):**

```tsx
import { createFileRoute, redirect } from "@tanstack/react-router";
import { ProtectedPage } from "@/features/myFeature/pages/ProtectedPage";
...
When adding new routes, simply create a new file in `src/routes/` and the Vite plugin will auto-generate the routing tree. Use `@tanstack/react-router` hooks for navigation (`useNavigate`, `useRouter`, `<Link to="...">`).
