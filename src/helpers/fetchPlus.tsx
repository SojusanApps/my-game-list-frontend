...
function fetchPlus(input: URL | RequestInfo, init: RequestInitPlus = {}) {
  ...
  const dispatchRequest = async (initModified: RequestInitPlus): Promise<ResponsePlus> => {
  ...
  const chain = [...requestInterceptors, { fulfilled: dispatchRequest, rejected: null }, ...responseInterceptors];
...
fetchPlus.requestInterceptors.use(
  ...
  () => Promise.reject(Error("An error occurred while setting the required request parameters.")),
);
