...
import { VisibilityEnum, ModeEnum, Friendship } from "@/client";
import { useCreateCollection, useFriendSearch } from "../hooks/useCollectionQueries";
...
const validationSchema = z.object({
  ...
  visibility: z.nativeEnum(VisibilityEnum).default(VisibilityEnum.PUB),
  mode: z.nativeEnum(ModeEnum).default(ModeEnum.S),
...
export default function CreateCollectionModal({ onClose }: Readonly<CreateCollectionModalProps>) {
  ...
  const handleToggleCollaborator = (friendship: Friendship, isSelected: boolean) => {
    if (isSelected) {
      ...
      <div className="flex flex-col h-full max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-8 py-6 border-b border-background-100 bg-background-50/50">
          <h2 className="text-2xl font-black text-text-900 tracking-tight">
            Create <span className="text-primary-600">Collection</span>
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-8">
          <FormProvider {...methods}>
            <form id="create-collection-form" onSubmit={methods.handleSubmit(onSubmit)} className="flex flex-col gap-6">
              <TextFieldInput label="Collection Name" name="name" placeholder="My Awesome Collection" required />
              ...
              <CheckboxInput label="Mark as Favorite" name="is_favorite" />
            </form>
          </FormProvider>
        </div>
